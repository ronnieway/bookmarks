...
angular.module('bookmarksApp')
	.controller('10Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[9].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('10/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/10/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/10/' + bookmark.id] = bookmarkUpdated;
