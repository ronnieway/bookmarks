...
angular.module('bookmarksApp')
	.controller('15Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[14].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('15/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/15/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/15/' + bookmark.id] = bookmarkUpdated;
