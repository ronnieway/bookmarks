...
angular.module('bookmarksApp')
	.controller('6Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[5].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('6/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/6/' + bookmark.id] = bookmarkUpdated;
