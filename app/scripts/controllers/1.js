...
angular.module('bookmarksApp')
	.controller('1Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[0].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('1/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/1/' + bookmark.id] = bookmarkUpdated;
