...
angular.module('bookmarksApp')
	.controller('5Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[4].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('5/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/5/' + bookmark.id] = bookmarkUpdated;
