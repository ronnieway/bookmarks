...
angular.module('bookmarksApp')
	.controller('7Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[6].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('7/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/7/' + bookmark.id] = bookmarkUpdated;
