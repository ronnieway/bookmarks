...
angular.module('bookmarksApp')
	.controller('9Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[8].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('9/').orderByKey();
		...
		function createBookmark(bookmark) {
			let newPostKey = firebase.database().ref().child('9').push().key;
		...
		function updateBookmark(bookmark) {
			...
			updates['/9/' + bookmark.id] = bookmarkUpdated;
