...
angular.module('bookmarksApp')
	.controller('13Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[12].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('13/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/13/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/13/' + bookmark.id] = bookmarkUpdated;
