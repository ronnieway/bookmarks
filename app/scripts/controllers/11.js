...
angular.module('bookmarksApp')
	.controller('11Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[10].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('11/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/11/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/11/' + bookmark.id] = bookmarkUpdated;
