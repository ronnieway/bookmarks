...
angular.module('bookmarksApp')
	.controller('3Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[2].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('3/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/3/' + bookmark.id] = bookmarkUpdated;
