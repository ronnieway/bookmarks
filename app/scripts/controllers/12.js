...
angular.module('bookmarksApp')
	.controller('12Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[11].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('12/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/12/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/12/' + bookmark.id] = bookmarkUpdated;
