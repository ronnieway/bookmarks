...
angular.module('bookmarksApp')
	.controller('4Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[3].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('4/').orderByKey();
		...
		function updateBookmark(bookmark) {
			...
			updates['/4/' + bookmark.id] = bookmarkUpdated;
