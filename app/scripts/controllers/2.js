...
angular.module('bookmarksApp')
	.controller('2Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout','catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];
	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[1].name;
		}, 1000);
		
	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('2/').orderByKey();
		...
		function createBookmark(bookmark) {
			let newPostKey = firebase.database().ref().child('2').push().key;
		...
		function updateBookmark(bookmark) {
			...
			updates['/2/' + bookmark.id] = bookmarkUpdated;
