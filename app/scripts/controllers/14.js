...
angular.module('bookmarksApp')
	.controller('14Ctrl', ['$scope', '$location', '$anchorScroll', '$timeout', 'catmanagement', function ($scope, $location, $anchorScroll, $timeout, catmanagement) {
	    $scope.bookmarks = [
	    ];

	    $timeout(function(){
		    $scope.cats = catmanagement;
		    $scope.theCatName = catmanagement[13].name;
		}, 1000);

	    var bookmarksArray = [];
	    new Promise(function(resolve) {
			let tref = firebase.database().ref('14/').orderByKey();
		...
		function createBookmark(bookmark) {
			...
			let updates = {};
			updates['/14/' + newPostKey] = bookmark;
		...
		function updateBookmark(bookmark) {
			...
			updates['/14/' + bookmark.id] = bookmarkUpdated;
